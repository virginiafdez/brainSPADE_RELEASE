...
from vqvae_ldm.training_and_testing.util import log_reconstructions, log_inferred_ldm
...
def eval_ae_kl_pv(
    ...
    for x_ind, x in enumerate(loader):
        ...
        with autocast(enabled=False):
            x_hat, z_mu, z_sigma = model(img)
            recon_loss = F.l1_loss(img.float(), x_hat.float())
