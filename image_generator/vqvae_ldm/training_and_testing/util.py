...
def get_training_data_loader_pv_conditioned_vol(
    ...
    val_transforms = transforms.Compose([
        ...
        transforms.SpatialPadd(
            keys=["label"],
            spatial_size=[192, 256, 135],
            method="symmetric",
            mode="edge"
        ),
        transforms.CenterSpatialCropd(
            keys=["label"],
            roi_size=[192, 256, 128]
        ),
        transforms.ConcatItemsd(keys=["label"], name="image"),
        transforms.ToTensord(keys=["image", ] + lesions)
    ])
...
def log_3d_img(
    ...
    for i in img_ids:
        ...
        for ch in range(rec_imgs.shape[1]):
            individual_channels.append(np.expand_dims((rec_imgs[i, ch, ...]*8).numpy().astype("uint8"), 0))
            individual_channels_gt.append(np.expand_dims((gt_imgs[i, ch, ...] * 8).numpy().astype("uint8"),0))
        ...
        monai.visualize.plot_2d_or_3d_image(data=out_img, step=step, writer=writer,
                                            index=0,
                                            tag="val_images_%d" %i, max_channels=1,
