...
class Modisc(nn.Module):
    ...
    def findLastModel(self, path_to_model, model_name = ''):
        '''
        ...
        '''
        if model_name:
            self.load_state_dict(torch.load(os.path.join(path_to_model, model_name)), strict=False)
        else:
    ...
    def saveLatestModel(self, path, epoch):

        torch.save(self.state_dict(), os.path.join(path, "%d_net_MD.pth" %epoch))
