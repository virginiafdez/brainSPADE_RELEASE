...
def saveFigs(imgs, fig_path, create_dir = False, nlabels = 1, index = -1,same_scale = False, titles = [],
    ...
    segmentation map is.
    :param bound_normalization: True or False, whether the normalization is between -1 and 1 or unbounded.
    :return:
    '''
    ...
    for b in range(batch_size):
        ...
        for ind_i, i in enumerate(b_imgs):
            plt.subplot(n_rows, n_columns, subplot_counter)
            if ind_i == index_label:
                plt.imshow(i)
                plt.title(titles_[ind_i], fontsize = 18)
                plt.axis('off')
            else:
                if same_scale:
                    plt.imshow(i, vmin=mins, vmax=maxes, cmap='gist_gray')
                else:
...
def saveSingleImage(img, label, path, skullstrip, denormalize = False,
    ...
    '''
    if denormalize and not islabel:
        img = (img + 1) / 2.0 * 255.0
    if skullstrip and not islabel:
        img = SkullStrip(img, label)
    if img.shape[0] < 4:
...
def SkullStrip(image, label, value = 0.0):
    ...
    :return:
    '''
    mask, negatives = Create2DMask(label[0, :, :], epsilon=0.65)

    for i in range(image.shape[0]):
        sub_i = image[i, :, :]
        sub_i = mask * sub_i
        sub_i[negatives] = value
        image[i, :, :] = sub_i
    return image

def Create2DMask(mask_base, sigma = 0.05, epsilon = 0.1):

    '''
    Creates a 2 dimensional mask from a label map, blurring the edges on the outside with a Gaussian Filter.
    :param self:
    :param mask_base: array containing the 2 dimensional label map
    :param sigma: optional, standard deviation for the Gaussian Filter
    :return:
    '''

    if mask_base.dim() > 2:
        ValueError("The mask dimension must be two-dimensional.")

    mask_base = mask_base.numpy()
    mask_base = 1*(mask_base < epsilon)
    mask_base = mask_base.astype(float)
    mask_copy = mask_base != 0
    mask_base = gaussian_filter(mask_base, sigma = sigma)
    mask_base[mask_copy] = 1.0
    negatives = ~ mask_copy
    return torch.from_numpy(mask_base).float(), torch.from_numpy(negatives)
...
def plot_D_outputs(outputs_D, save_path, namefig, title, bound_normalization = False):
    '''
    ...
    for d_ind, d in enumerate(pred_real):
        for b in range(n_rows):
            ...
            disc_outs[b]['fake'].append(util.tensor2im(pred_fake[d_ind][b, ...],bound_normalization = bound_normalization))
            disc_outs[b]['real'].append(util.tensor2im(d[b, ...], bound_normalization = bound_normalization))
    ...
    for b in range(n_rows):
        ...
        img_real = util.tensor2im(gt[b, ...], bound_normalization = bound_normalization)
        img_fake = util.tensor2im(fake[b, ...], bound_normalization = bound_normalization)
...
def blurSTD(label, num_threshold = 30, minimal_area = 10, ignore_last = 2):
    ...
    If the number of measurements (runnable_bash_files) is above threshold, we median filter it.
    :param label: HxWx1xC (C: Channels) numpy array
    :param threshold: threshold above which we perform the blur. Number of runnable_bash_files
