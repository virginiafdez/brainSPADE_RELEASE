""" Script to preprocess the images from OASIS using nipype.
...
def main(args):
    ...
    derivatives_dir = Path("/targetdata/OASIS_ANTs_RegistrationSynQuick/")
    ...
    for i, participant in enumerate(participant_list[args.start:args.stop]):
        ...
        session_id = participant.split("_")[1]
        run_id = participant.split("_")[2]

        t1_image = sourcedata_t1_dir / f"T1_{participant_id}_{session_id}_{run_id}.nii.gz"
        flair_image = sourcedata_flair_dir / f"FLAIR_{participant_id}_{session_id}_{run_id}.nii.gz"
        parcellation_image = sourcedata_parcellation_dir / f"Parcellation_{participant_id}_{session_id}_{run_id}.nii.gz"
        segmentation_image = sourcedata_segmentation_dir / f"Parcellation_{participant_id}_{session_id}_{run_id}.nii.gz"
        ...
        try:
            ...
            reg.inputs.output_prefix = str(
                derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_"
            )
            ...
            at1.inputs.output_image = str(
                derivatives_flair_dir / f"FLAIR_{participant_id}_{session_id}_{run_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
            ...
            at1.inputs.reference_image = str(template_image)
            at1.inputs.output_image = str(
                derivatives_parcellation_dir / f"Parcellation_{participant_id}_{session_id}_{run_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
            ...
            at1.inputs.reference_image = str(template_image)
            at1.inputs.output_image = str(
                derivatives_segmentation_dir / f"Parcellation_{participant_id}_{session_id}_{run_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
        ...
        t1_reg_file = derivatives_t1_dir / f"T1_{participant_id}_{session_id}_{run_id}_space-{space_name}.nii.gz"
        (derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_Warped.nii.gz").rename(t1_reg_file)

        os.remove(str(derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_0GenericAffine.mat"))
        os.remove(str(derivatives_dir / f"{participant_id}_{session_id}_{run_id}_space-{space_name}_T1w_InverseWarped.nii.gz"))
