...
def main(args):
    ...
    derivatives_dir = Path("/targetdata/BRATS-OTHER_ANTs_RegistrationSynQuick/")
    ...
    for i, participant in enumerate(participant_list[args.start:args.stop]):
        ...
        t1_image = sourcedata_t1_dir / f"T1_BraTS20_BRATS-OTHER_Training_{participant_id}.nii.gz"
        t2_image = sourcedata_t2_dir / f"T2_BraTS20_BRATS-OTHER_Training_{participant_id}.nii.gz"
        flair_image = sourcedata_flair_dir / f"FLAIR_BraTS20_BRATS-OTHER_Training_{participant_id}.nii.gz"
        parcellation_image = sourcedata_parcellation_dir / f"Parcellation_BraTS20_BRATS-OTHER_Training_{participant_id}.nii.gz"
        segmentation_image = sourcedata_segmentation_dir / f"Parcellation_BraTS20_BRATS-OTHER_Training_{participant_id}.nii.gz"
