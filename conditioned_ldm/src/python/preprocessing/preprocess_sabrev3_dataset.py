""" Script to preprocess the images from SABREv3 using nipype.
...
def main(args):
    ...
    derivatives_dir = Path("/targetdata/SABREV3_ANTs_RegistrationSynQuick/")
    ...
    sourcedata_b0_dir = sourcedata_dir / "B0"
    ...
    derivatives_b0_dir = derivatives_dir / "B0"
    ...
    derivatives_b0_dir.mkdir(exist_ok=True)
    ...
    for i, participant in enumerate(participant_list[args.start:args.stop]):
        ...
        session_id = participant.split("_")[2]

        t1_image = sourcedata_t1_dir / f"T1_SABRE_{participant_id}_{session_id}.nii.gz"
        t2_image = sourcedata_t2_dir / f"T2_SABRE_{participant_id}_{session_id}.nii.gz"
        flair_image = sourcedata_flair_dir / f"FLAIR_SABRE_{participant_id}_{session_id}.nii.gz"
        b0_image = sourcedata_b0_dir / f"B0_SABRE_{participant_id}_{session_id}.nii.gz"
        parcellation_image = sourcedata_parcellation_dir / f"Parcellation_SABRE_{participant_id}_{session_id}.nii.gz"
        segmentation_image = sourcedata_segmentation_dir / f"Parcellation_SABRE_{participant_id}_{session_id}.nii.gz"
        ...
        try:
            ...
            reg.inputs.output_prefix = str(
                derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_"
            )
            ...
            at1.inputs.output_image = str(
                derivatives_t2_dir / f"T2_{participant_id}_{session_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
            ...
            at1.inputs.reference_image = str(template_image)
            at1.inputs.output_image = str(
                derivatives_flair_dir / f"FLAIR_{participant_id}_{session_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
            ...
            # Apply transformation to register B0 to MNI space
            at1 = ApplyTransforms()
            at1.inputs.dimension = 3
            at1.inputs.input_image = str(b0_image)
            at1.inputs.reference_image = str(template_image)
            at1.inputs.output_image = str(
                derivatives_b0_dir / f"B0_{participant_id}_{session_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
            ...
            at1.inputs.reference_image = str(template_image)
            at1.inputs.output_image = str(
                derivatives_parcellation_dir / f"Parcellation_{participant_id}_{session_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
            ...
            at1.inputs.reference_image = str(template_image)
            at1.inputs.output_image = str(
                derivatives_segmentation_dir / f"Parcellation_{participant_id}_{session_id}_space-{space_name}.nii.gz"
            )
            at1.inputs.transforms = [
                str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_0GenericAffine.mat")
            ]
        ...
        t1_reg_file = derivatives_t1_dir / f"T1_{participant_id}_{session_id}_space-{space_name}.nii.gz"
        (derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_Warped.nii.gz").rename(t1_reg_file)

        os.remove(str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_0GenericAffine.mat"))
        os.remove(str(derivatives_dir / f"{participant_id}_{session_id}_space-{space_name}_T1w_InverseWarped.nii.gz"))
